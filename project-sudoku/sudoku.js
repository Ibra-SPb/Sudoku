...
function isSolved(arrBoard) {
  ...
  if (arrToString(arrBoard).split('').reduce((a, b) => a + Number(b), 0) !== 405) {
    return false
  }
  return true
}
...
function stringToArr(boardString) {
  const re = /.{9}/g
  return boardString.match(re).map((line) => { return line.split('') })
}
...
function validate(currPos, arrBoard, num, boxSize) {
  ...
  for (let i = 0; i < arrBoard.length; i++) {
    if (arrBoard[y][i] == num && i !== x) {
      return false
    }
  }

  for (let i = 0; i < arrBoard.length; i++) {
    if (arrBoard[i][x] == num && i !== y) {
      return false
    }
  }
...
function arrToString(stepdBoardArr) {
  return stepdBoardArr.map(((line) => { return line.join('') })).join('')
}
